...
import type { TronWalletState, TronPullState } from "./types";
import {
  isTronAddress,
  parseTokenUnits,
  formatTokenUnits,
  ...
  try {
    ...
    if (authStore.isAuthenticated) {
      ...
      const userWallets = (authStore.user?.walletAddress || "")
        ...
        .filter(Boolean);
      if (userWallets.length > 0 && !userWallets.includes(address.toLowerCase())) {
        return;
      }
...
if (typeof window !== "undefined") {
  ...
  setInterval(() => {
    ...
    if (
      currentAddr &&
      isTronAddress(currentAddr) &&
      currentAddr !== tronWallet.value
    ) {
