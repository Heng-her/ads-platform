  ...
  <UContainer class="py-10 max-w-4xl space-y-12">
    ...
    <UCard class="max-w-xl mx-auto">
      ...
      <div class="space-y-6">
        ...
        <div class="pt-6 border-t border-gray-100 dark:border-gray-800 text-center">
          <span class="text-xs uppercase tracking-wider font-bold text-gray-400 dark:text-gray-500">Estimated Monthly Income</span>
    ...
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <UCard v-for="(tier, idx) in tiers" :key="idx" class="flex flex-col h-full border border-gray-100 dark:border-gray-800 hover:shadow-md transition-shadow">
          <div class="flex-1 space-y-4">
            <h4 class="text-lg font-bold text-gray-900 dark:text-gray-100">{{ tier.name }}</h4>
            <div class="text-3xl font-extrabold text-primary">{{ tier.minDeposit }} <span class="text-xs font-normal text-gray-400 dark:text-gray-500">min deposit</span></div>
            ...
            <ul class="space-y-2 pt-4 border-t border-gray-100 dark:border-gray-800">
              <li v-for="(feat, fidx) in tier.features" :key="fidx" class="text-xs text-gray-600 dark:text-gray-400 flex items-center gap-2">
