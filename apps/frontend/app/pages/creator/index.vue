...
function calculateEstimatedEarnings(impressions: number): string {
  // Estimated eCPM rate $2.50 per 1,000 impressions
  const est = (impressions / 1000) * 2.5
  return new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(est)
}
  ...
  <div class="mx-auto max-w-7xl space-y-8 pb-12">
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      <div class="flex items-center gap-4">
        <div class="relative h-14 w-14 overflow-hidden rounded-full border-2 border-primary-500/40 bg-gray-100 dark:bg-gray-800 shadow-md">
          <img v-if="authStore.user?.avatar" :src="authStore.user.avatar" :alt="creatorName" class="h-full w-full object-cover" />
          <div v-else class="flex h-full w-full items-center justify-center text-gray-400">
            <UIcon name="i-heroicons-sparkles" class="h-8 w-8 text-amber-500" />
          </div>
        </div>
        <div>
    ...
    <div v-if="isLoading && !stats" class="grid gap-6 md:grid-cols-3">
      <div v-for="i in 3" :key="i" class="h-36 animate-pulse rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900" />
    </div>
    ...
    <div v-else-if="stats" class="grid gap-6 md:grid-cols-3">
      ...
      <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        <div class="flex items-center justify-between">
          <span class="text-xs font-semibold uppercase tracking-wider text-primary-600 dark:text-primary-400">Campaigns</span>
        ...
        <div class="mt-3 flex items-baseline gap-2">
          <span class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">{{ formatNumber(stats.campaigns.total) }}</span>
        ...
        <div class="mt-4 flex items-center justify-between border-t border-gray-100 pt-3 text-xs dark:border-gray-800">
          <span class="text-gray-600 dark:text-gray-400">Public: <strong class="text-emerald-600 dark:text-emerald-400">{{ formatNumber(stats.campaigns.public) }}</strong></span>
          <span class="text-gray-600 dark:text-gray-400">Draft: <strong class="text-amber-600 dark:text-amber-400">{{ formatNumber(stats.campaigns.draft) }}</strong></span>
        </div>
      </div>
      ...
      <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        ...
        <div class="mt-3 flex items-baseline gap-2">
          <span class="text-3xl font-bold tracking-tight text-sky-600 dark:text-sky-400">{{ formatNumber(stats.impressions.total) }}</span>
        ...
        <div class="mt-4 flex items-center justify-between border-t border-gray-100 pt-3 text-xs text-gray-500 dark:border-gray-800 dark:text-gray-400">
          <span>Unique Viewers Reach: <strong class="text-gray-900 dark:text-white">{{ formatNumber(stats.impressions.uniqueViewers) }}</strong></span>
        </div>
      </div>
      ...
      <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        <div class="flex items-center justify-between">
          <span class="text-xs font-semibold uppercase tracking-wider text-amber-600 dark:text-amber-400">Est. Revenue</span>
        ...
        <div class="mt-3 flex items-baseline gap-2">
          <span class="text-3xl font-bold tracking-tight text-amber-600 dark:text-amber-400">
            {{ calculateEstimatedEarnings(stats.impressions.total) }}
          </span>
        </div>
        <div class="mt-4 flex items-center justify-between border-t border-gray-100 pt-3 text-xs text-gray-500 dark:border-gray-800 dark:text-gray-400">
          <span>eCPM Rate: <strong class="text-gray-900 dark:text-white">$2.50 / 1k views</strong></span>
        </div>
      </div>
    </div>
    ...
    <div v-if="stats" class="grid gap-8 lg:grid-cols-3">
      ...
      <div class="space-y-8 lg:col-span-2">
        ...
        <div class="relative overflow-hidden rounded-xl border border-primary-500/30 bg-gradient-to-r from-primary-950/20 via-gray-900 to-gray-900 p-6 shadow-sm text-white">
          ...
          <div v-else class="mt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-6 rounded-lg bg-gray-950/60 p-4 border border-gray-800 shrink-0">
              <div>
                <p class="text-lg font-extrabold text-sky-400">{{ formatNumber(stats.topCampaign.totalImpressions) }}</p>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
          <div class="flex items-center justify-between border-b border-gray-100 pb-4 dark:border-gray-800">
            ...
            <NuxtLink to="/creator/campaigns" class="text-xs font-medium text-primary-600 hover:text-primary-500 dark:text-primary-400">
              View All &rarr;
            </NuxtLink>
          </div>
          ...
          <div v-else class="mt-4 divide-y divide-gray-100 dark:divide-gray-800">
            <div
              v-for="campaign in stats.recentCampaigns"
              :key="campaign.id"
            ...
            >
              ...
              <div class="flex items-center gap-3 shrink-0 ml-4">
                <UBadge
                  :color="campaign.status === 'PUBLIC' ? 'success' : 'neutral'"
                  variant="soft"
                  size="xs"
                  class="font-semibold"
                >
      ...
      <div class="space-y-8 lg:col-span-1">
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900 space-y-4">
          <h2 class="text-sm font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 border-b border-gray-100 pb-3 dark:border-gray-800">
