...
<template>
  <div class="space-y-6">
    <div
      class="flex flex-col gap-4 border-b border-gray-200 pb-4 sm:flex-row sm:items-center sm:justify-between dark:border-gray-800">
      <div>
        <p class="text-xs font-semibold uppercase tracking-wider text-primary-600 dark:text-primary-400">Campaign
          oversight</p>
        <h1 class="mt-1 text-xl font-bold text-gray-900 dark:text-gray-100">Users &amp; published posts</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Review every user, their three latest public posts, and
          their complete campaign history.</p>
      </div>
      <UButton to="/creator/campaigns/create" color="primary" icon="i-heroicons-plus" size="sm">New Campaign</UButton>
    </div>

    <div
      class="flex flex-col gap-3 rounded-xl border border-gray-200 bg-white p-3 sm:flex-row sm:items-center sm:justify-between dark:border-gray-800 dark:bg-gray-900">
      <UInput v-model="searchQuery" icon="i-heroicons-magnifying-glass" placeholder="Search users by name or email..."
        size="sm" class="w-full sm:max-w-md" />
      ...
      <article v-for="user in users" :key="user.id"
        ...
        <header class="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div class="flex min-w-0 items-center gap-3">
            ...
            <div v-else
              class="flex h-11 w-11 items-center justify-center rounded-full bg-primary-100 font-semibold text-primary-700 dark:bg-primary-950 dark:text-primary-300">
            ...
            <div class="min-w-0">
              ...
              <p class="truncate text-xs text-gray-500 dark:text-gray-400">{{ user.email }} · Campaigns load on demand
              </p>
            </div>
          </div>
          <UButton color="neutral" variant="outline" size="sm"
            :icon="expandedUserIds.has(user.id) ? 'i-heroicons-chevron-up' : 'i-heroicons-chevron-down'"
        ...
        <section class="border-t border-gray-100 bg-gray-50/70 p-4 dark:border-gray-800 dark:bg-gray-950/30">
          ...
          <div v-if="user.publicPosts.length" class="grid gap-3 md:grid-cols-6">
            <NuxtLink v-for="post in user.publicPosts" :key="post.id" :to="getArticleUrl(post)" target="_blank"
              class="group overflow-hidden rounded-lg border border-gray-200 bg-white transition hover:border-primary-300 hover:shadow-sm dark:border-gray-800 dark:bg-gray-900">
              <img v-if="post.imageUrl" :src="post.imageUrl" :alt="post.imageTitle || post.title"
                class="h-28 w-full object-cover" />
              <div v-else class="flex h-28 items-center justify-center bg-gray-100 text-gray-400 dark:bg-gray-800">
                <UIcon name="i-heroicons-photo" class="h-6 w-6" />
              </div>
              <div class="p-3">
                <p
                  class="line-clamp-1 text-sm font-semibold text-gray-900 group-hover:text-primary-600 dark:text-gray-100">
                  {{ post.title }}</p>
                <p class="mt-1 text-xs text-gray-500">{{ post.category || 'General' }} · {{ new
                  Date(post.createdAt).toLocaleDateString() }}</p>
              </div>
            </NuxtLink>
          </div>
          <p v-else
            class="rounded-lg border border-dashed border-gray-200 bg-white px-3 py-2 text-center text-sm text-gray-500 dark:border-gray-800 dark:bg-gray-900">
            This user has no public posts yet.</p>
        </section>
        ...
        <section v-if="expandedUserIds.has(user.id)" class="border-t border-gray-200 dark:border-gray-800">
          <div class="no-scrollbar overflow-x-auto">
            <table class="w-full min-w-[720px] text-left text-xs">
              ...
              <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                <tr v-for="campaign in campaignsFor(user.id)" :key="campaign.id"
                  class="hover:bg-gray-50 dark:hover:bg-gray-800/40">
                  <td class="px-4 py-3">
                    <div class="flex items-center gap-3"><img v-if="campaign.imageUrl" :src="campaign.imageUrl" alt=""
                        class="h-9 w-9 rounded object-cover" />
                      <div v-else class="h-9 w-9 rounded bg-gray-100 dark:bg-gray-800" /><span
                        class="max-w-64 truncate font-medium text-gray-900 dark:text-gray-100">{{ campaign.title
                        }}</span>
                    </div>
                  </td>
                  ...
                  <td class="px-4 py-3">
                    <CampaignRowActions :article-url="getArticleUrl(campaign)"
                      ...
                      :deleting="deletingCampaignIds.has(campaign.id)" @delete="removeCampaign(user, campaign)" />
                  </td>
                </tr>
                <tr v-if="loadingCampaignUserIds.has(user.id)">
                  <td colspan="5" class="px-4 py-8 text-center text-sm text-gray-500">Loading campaigns...</td>
                </tr>
                <tr v-else-if="!campaignsFor(user.id).length">
          ...
          <div v-if="campaignPageFor(user.id) && campaignPageFor(user.id)!.totalPages > 1"
            class="flex items-center justify-between border-t border-gray-100 px-4 py-3 text-xs dark:border-gray-800">
            <span class="text-gray-500">Campaign page {{ campaignPageFor(user.id)!.page }} of {{
              campaignPageFor(user.id)!.totalPages }}</span>
            <div class="flex gap-2">
              <UButton color="neutral" variant="outline" size="xs" :disabled="campaignPageFor(user.id)!.page === 1"
                @click="loadUserCampaigns(user.id, campaignPageFor(user.id)!.page - 1)">Previous</UButton>
              <UButton color="neutral" variant="outline" size="xs"
                :disabled="campaignPageFor(user.id)!.page === campaignPageFor(user.id)!.totalPages"
                @click="loadUserCampaigns(user.id, campaignPageFor(user.id)!.page + 1)">Next</UButton>
            </div>
          </div>
        </section>
      </article>
