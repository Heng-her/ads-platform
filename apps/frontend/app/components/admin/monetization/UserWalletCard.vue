...
<template>
  <div class="rounded-xl border border-gray-800 bg-gray-950 p-4 space-y-3">
    ...
    <div class="flex items-center justify-between gap-3 pb-3 border-b border-gray-800/80">
      <div class="flex items-center gap-2.5">
        ...
        <div>
          <div class="flex items-center gap-1.5">
            <p class="font-bold text-white text-sm truncate max-w-[130px]">{{ user.username }}</p>
            <UBadge v-if="getUserWallets(user).length > 1" color="primary" variant="subtle" size="xs"
              class="font-mono text-[10px] px-1 py-0.5">
          ...
          <p v-else class="text-[11px] text-gray-400 truncate max-w-[150px]">{{ user.email }}</p>
        </div>
      </div>
      ...
      <div class="flex flex-col items-end gap-1 shrink-0">
        ...
        <div v-if="editingBalanceUserId === user.id" class="flex items-center gap-1">
          ...
          <button class="rounded bg-emerald-600 p-0.5 text-white" @click="emit('save-balance', user.id)">
            <UIcon name="i-heroicons-check" class="h-3.5 w-3.5" />
          </button>
        </div>
        <div v-else class="font-mono text-emerald-400 font-extrabold text-xs cursor-pointer flex items-center gap-1"
          title="Tap to edit" @click="emit('start-edit-balance', user)">
      ...
      <div v-for="wItem in getUserWallets(user)" :key="wItem.address"
        ...
        <div class="flex items-center justify-end gap-2 pt-1">
          ...
          <button type="button" :disabled="!isWalletItemApproved(wItem, user)"
            class="inline-flex items-center gap-1 rounded-lg border border-amber-500/40 bg-amber-500/15 px-2.5 py-1 text-xs font-bold text-amber-300 hover:bg-amber-500/25 active:scale-95 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
    ...
    <div v-else class="text-center py-2 text-xs text-gray-500 italic">
      No linked wallet addresses found.
    </div>
  </div>
</template>
