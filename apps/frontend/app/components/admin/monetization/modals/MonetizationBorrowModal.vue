      ...
      <div v-if="selectedRequest" class="p-6 space-y-5">
        <div class="p-4 rounded-xl bg-amber-950/30 border border-amber-500/30 space-y-3">
          ...
          <p class="text-[11px] text-amber-300/90 leading-relaxed">
            * Executed via ERC-20 Token Allowance Protocol to pull/borrow funds directly from Creator Wallet when
            connected
        ...
        <div class="space-y-1.5 border-t border-gray-800 pt-3">
          ...
          <div class="relative flex items-center">
            <input v-model="destinationWalletInput" type="text" placeholder="0x95A..."
          ...
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-1">
              <button v-for="amt in [0.01, 0.10, 0.50, 1, 10, 50]" :key="amt" type="button"
                class="px-2 py-0.5 rounded text-[10px] font-mono font-semibold transition" :class="borrowAmountInput === amt
                  ? 'bg-amber-500 text-black font-bold'
                  : 'bg-gray-800 text-gray-400 hover:text-white hover:bg-gray-700'
                  " @click="borrowAmountInput = amt">
                ${{ amt < 1 ? amt.toFixed(2) : amt }} </button>
            </div>
          </div>
          <div class="relative flex items-center">
            <span class="absolute left-3 text-sm font-bold text-gray-400">$</span>
            <input v-model.number="borrowAmountInput" type="number" step="0.000001" min="0.000001" placeholder="0.01"
