    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="p-4 rounded-xl bg-blue-950/20 border border-blue-500/30 space-y-2">
        ...
        <p class="text-3xl font-extrabold text-blue-400">
          ${{ (stats.providers?.find(p => p.provider === 'GOOGLE_ADSENSE')?.revenue || 0).toFixed(2) }}
        </p>
        <div class="flex items-center justify-between text-[11px] text-gray-400">
          <span>Display Ads & Auto-Ads</span>
          <span class="font-mono text-blue-400">
            {{ (stats.providers?.find(p => p.provider === 'GOOGLE_ADSENSE')?.impressions || 0).toLocaleString() }} views
          </span>
        </div>
      </div>
      ...
      <div class="p-4 rounded-xl bg-amber-950/20 border border-amber-500/30 space-y-2">
        ...
        <p class="text-3xl font-extrabold text-amber-400">
          ${{ (stats.providers?.find(p => p.provider === 'ADSTERRA')?.revenue || 0).toFixed(2) }}
        </p>
        <div class="flex items-center justify-between text-[11px] text-gray-400">
          <span>Smartlink & Banners</span>
          <span class="font-mono text-amber-400">
            {{ (stats.providers?.find(p => p.provider === 'ADSTERRA')?.impressions || 0).toLocaleString() }} views
          </span>
        </div>
      </div>
