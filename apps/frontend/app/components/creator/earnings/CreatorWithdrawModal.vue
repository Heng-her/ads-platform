    ...
    <template #content>
      <div class="p-6 space-y-5 bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800">
        ...
        <div class="space-y-1">
          ...
          <div v-if="stats.availableBalance <= 0"
            class="rounded-xl bg-amber-500/10 border border-amber-500/30 p-3.5 space-y-1 mt-2">
            <div class="flex items-start gap-2.5">
              ...
              <div>
                ...
                <p class="text-[11px] text-gray-300 leading-relaxed mt-0.5">
                  You do not have enough earnings available to withdraw yet. Minimum payout threshold is <span
                    class="font-bold text-white">${{ stats.minPayoutThreshold || 20 }}</span>.
                </p>
              </div>
            </div>
          </div>

          <div v-else-if="stats.availableBalance < (stats.minPayoutThreshold || 20)"
            class="rounded-xl bg-amber-500/10 border border-amber-500/30 p-3.5 space-y-1 mt-2">
            <div class="flex items-start gap-2.5">
              <UIcon name="i-heroicons-exclamation-triangle" class="w-5 h-5 text-amber-400 shrink-0 mt-0.5" />
              <div>
                ...
                <p class="text-[11px] text-gray-300 leading-relaxed mt-0.5">
                  Your available balance is <span class="font-bold text-white">{{ formatCurrency(stats.availableBalance)
                  }}</span>. Minimum threshold required for withdrawal is <span class="font-bold text-amber-400">${{
                      stats.minPayoutThreshold || 20 }}</span>.
                </p>
              </div>
            </div>
          </div>

          <div v-else-if="withdrawAmount > stats.availableBalance"
            class="rounded-xl bg-red-500/10 border border-red-500/30 p-3.5 space-y-1 mt-2">
            <div class="flex items-start gap-2.5">
              ...
              <div>
                ...
                <p class="text-[11px] text-gray-300 leading-relaxed mt-0.5">
                  Requested amount of <span class="font-bold text-white">${{ withdrawAmount }}</span> exceeds your
                  available balance of <span class="font-bold text-amber-400">{{ formatCurrency(stats.availableBalance)
                  }}</span>.
                </p>
              </div>
            </div>
