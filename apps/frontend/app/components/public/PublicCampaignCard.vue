...
<template>
  <div class="block">
    <article
      ...
      <!-- Full-Width Media Image Banner Container (Only shown if image exists) -->
      <NuxtLink v-if="galleryImages.length > 0 && galleryImages[0]" :to="getArticleUrl(campaign)" class="block w-full">
        <div
          ...
          <NuxtImg :src="galleryImages[0]" :alt="campaign.imageTitle || campaign.title"
            loading="lazy"
      ...
      <!-- Stats & Share Action Row (Placed Below Image) -->
      <div class="flex items-center gap-3 text-xs font-medium text-gray-500 dark:text-gray-400 pt-0.5">
