...
const mobileNavItems = [
  { label: 'New', icon: 'clarity:new-line', to: '/' },
  { label: 'Explore', icon: 'i-heroicons-sparkles', to: '/explore' },
  { label: 'Trending', icon: 'i-heroicons-fire', to: '/trending' },
...
<template>
  <div class="min-h-screen flex flex-col bg-background text-foreground relative">
    ...
    <AppHeader>
      ...
      <template #center>
        <nav
          class="hidden md:flex items-center gap-1 bg-gray-100/70 dark:bg-gray-800 p-1 rounded-full border border-gray-200/50 dark:border-gray-800/50 shadow-inner">
        ...
        <div class="hidden md:flex items-center gap-3">
          ...
          <div ref="searchWrapper" class="relative hidden md:block">
            ...
            <div v-if="isPopoverOpen"
              ...
              <div v-if="searchQuery.trim().length === 0" class="space-y-3">
                <span class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Try
                  searching for...</span>
                <div class="flex flex-wrap gap-1.5">
                  <UButton v-for="sug in ['AI', 'Finance', 'News', 'Creator', 'Admin']" :key="sug" size="xs"
                    color="neutral" variant="subtle" @click="searchQuery = sug">
                    {{ sug }}
                  </UButton>
                </div>
              </div>
              ...
              <div v-else class="space-y-3">
                <span class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Search
                  Results</span>

                <div v-if="filteredResults.length === 0" class="text-xs text-gray-500 dark:text-gray-400">
                  No matches found for "{{ searchQuery }}"
                </div>

                <div v-else class="space-y-1 max-h-60 overflow-y-auto">
                  <div v-for="res in filteredResults" :key="res.label"
                    ...
                    <UIcon :name="res.icon" class="w-5 h-5 text-primary shrink-0" />
    ...
    <div v-if="showMobileSearch"
      ...
      <div class="h-16 flex items-center px-4 gap-2 shrink-0">
        <UInput v-model="searchQuery" icon="i-heroicons-magnifying-glass" placeholder="Search..." class="flex-1"
          autofocus />
        <UButton label="Cancel" color="neutral" variant="ghost" @click="showMobileSearch = false" />
      </div>

      <!-- Mobile Suggestions Dropdown -->
      <div
        class="p-4 border-t border-gray-100 dark:border-gray-850 space-y-4 bg-white dark:bg-gray-900 max-h-[calc(100vh-4rem)] overflow-y-auto">
        <!-- If search is empty, show quick navigation suggestions -->
        <div v-if="searchQuery.trim().length === 0" class="space-y-3">
          <span class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Try searching
            for...</span>
          <div class="flex flex-wrap gap-1.5">
            <UButton v-for="sug in ['AI', 'Finance', 'News', 'Creator', 'Admin']" :key="sug" size="xs" color="neutral"
              variant="subtle" @click="searchQuery = sug">
              {{ sug }}
            </UButton>
          </div>
        </div>
    ...
    <nav
      ...
      <NuxtLink v-for="item in mobileNavItems" :key="item.to" :to="item.to"
        class="flex flex-col items-center justify-center flex-1 h-full transition-colors py-1" :class="[
          $route.path === item.to
